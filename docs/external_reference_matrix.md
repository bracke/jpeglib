...
| Lossless Huffman RGB encode | `tests/bin/jpeglib_decode_raw`; `ffmpeg`; ImageMagick `magick` | Required native process oracle; required third-party `ffmpeg` oracle; ImageMagick diagnostic | Encoded artifacts must decode through the separate raw-decoder process and `ffmpeg` raw RGB output; external ImageMagick acceptance is reported when the host decoder supports the mode. |
...
conversion bytes. Lossless Huffman RGB rows also fail if `ffmpeg` cannot decode
the generated artifact to matching raw RGB bytes.
...
requires matching RGB conversion bytes. For lossless Huffman RGB artifacts the
gate also launches `ffmpeg` and requires matching raw RGB bytes.
