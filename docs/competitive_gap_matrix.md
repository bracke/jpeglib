...
| Performance architecture | Scalar reference benchmarks and hostkit-backed optional platform acceleration hooks. | Closed by `jpeglib_performance_matrix`, which uses `../hostkit` to report the build host and runs scalar baseline, progressive, and arithmetic encode/decode loops with deterministic output equivalence and runtime thresholds; complete-plus enforces the scalar-reference platform profile. |
