...
| Performance architecture | SIMD-capable color hot paths, scalar-equivalence checks, and hostkit-backed platform reporting. | Closed by `Jpeglib.Internal.Colors` compiler-vectorized row RGB-to-YCbCr, YCbCr-to-RGB-family output, and direct RGB row output kernels; encoder plane filling plus direct full-size lossless and 4:4:4 DCT decode row output use those kernels; `Jpeglib.Capabilities.SIMD_Acceleration` advertises the surface; `jpeglib_simd_matrix` checks bit-exact scalar equivalence across RGB/BGR/RGBA/BGRA input storage and every public output format; `jpeglib_performance_matrix` reports host-backed encode/decode thresholds. |
