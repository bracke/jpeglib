...
| Performance architecture | SIMD-capable color hot paths, scalar-equivalence checks, and hostkit-backed platform reporting. | Closed by `Jpeglib.Internal.Colors` compiler-vectorized row RGB-to-YCbCr and YCbCr-to-RGB-family output kernels, encoder plane filling and direct full-size lossless YCbCr decode row output using those kernels, `Jpeglib.Capabilities.SIMD_Acceleration`, `jpeglib_simd_matrix` bit-exact scalar equivalence checks across RGB/BGR/RGBA/BGRA input and output storage, and `jpeglib_performance_matrix` host-reported encode/decode thresholds. |
