...
procedure Jpeglib_Stress_Matrix is
   ...
   Manifest_Relative : constant String := "tests/fixtures/stress/streaming_matrix.txt";
   Errors : Natural := 0;
   Entries : Natural := 0;
   Open_Rows : Natural := 0;
   ...
   begin
      Errors := Errors + 1;
      Ada.Text_IO.Put_Line (Ada.Text_IO.Standard_Error, "jpeglib_stress_matrix: " & Message);
   end Fail;
   ...
   begin
      ...
      while not Ada.Text_IO.End_Of_File (File) loop
         ...
         begin
            if Text = "# id|surface|stress_scope|limit_policy|status|evidence" then
               Header_Found := True;
            elsif Text'Length > 0 and then Text (Text'First) /= '#' then
               Entries := Entries + 1;
               if Separator_Count (Text) /= 5 then
                  Fail ("stress row has wrong column count: " & Text);
               elsif Field (Text, 5) = "open" then
                  Open_Rows := Open_Rows + 1;
                  if not Allow_Open then
                     Fail ("stress row remains open: " & Field (Text, 1));
                  end if;
               elsif Field (Text, 5) /= "closed" then
                  Fail ("stress row has invalid status: " & Text);
               end if;
            end if;
         end;
      end loop;
      ...
      if not Header_Found then
         Fail ("stress matrix header is missing required columns");
      end if;
      if Entries = 0 then
         Fail ("stress matrix has no entries");
      end if;
   exception
...
begin
   if Root = "" then
      Ada.Text_IO.Put_Line (Ada.Text_IO.Standard_Error, "jpeglib_stress_matrix: run below the jpeglib tree");
   ...
   then
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error,
         "usage: jpeglib_stress_matrix [--allow-open]");
      Ada.Command_Line.Set_Exit_Status
        (Ada.Command_Line.Exit_Status (Jpeglib_Tools.Code (Jpeglib_Tools.Invalid_Command)));
      return;
   end if;
   ...
   if Errors = 0 then
      Ada.Text_IO.Put_Line
        ("jpeglib_stress_matrix: matrix passed with"
      ...
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error,
         "jpeglib_stress_matrix: " & Natural'Image (Errors) & " matrix issue(s)");
      Ada.Command_Line.Set_Exit_Status
        (Ada.Command_Line.Exit_Status (Jpeglib_Tools.Code (Jpeglib_Tools.Policy_Failure)));
   end if;
end Jpeglib_Stress_Matrix;
