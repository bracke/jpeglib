...
procedure Jpeglib_Policy_Matrix is
   ...
   Manifest_Relative : constant String := "tests/fixtures/policy/public_api_matrix.txt";
   Errors : Natural := 0;
   Entries : Natural := 0;
   Open_Rows : Natural := 0;
   ...
   begin
      Errors := Errors + 1;
      Ada.Text_IO.Put_Line (Ada.Text_IO.Standard_Error, "jpeglib_policy_matrix: " & Message);
   end Fail;
   ...
   begin
      ...
      if not Header_Found then
         Fail ("matrix header is missing required columns");
      end if;
      if Entries = 0 then
         Fail ("policy matrix has no entries");
      end if;
   exception
...
begin
   if Root = "" then
      Ada.Text_IO.Put_Line (Ada.Text_IO.Standard_Error, "jpeglib_policy_matrix: run below the jpeglib tree");
   ...
   then
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error,
         "usage: jpeglib_policy_matrix [--allow-open]");
      Ada.Command_Line.Set_Exit_Status
        (Ada.Command_Line.Exit_Status (Jpeglib_Tools.Code (Jpeglib_Tools.Invalid_Command)));
      return;
   end if;
   ...
   if Errors = 0 then
      Ada.Text_IO.Put_Line
        ("jpeglib_policy_matrix: matrix passed with"
      ...
      Ada.Text_IO.Put_Line
        (Ada.Text_IO.Standard_Error,
         "jpeglib_policy_matrix: " & Natural'Image (Errors) & " matrix issue(s)");
      Ada.Command_Line.Set_Exit_Status
        (Ada.Command_Line.Exit_Status (Jpeglib_Tools.Code (Jpeglib_Tools.Policy_Failure)));
   end if;
end Jpeglib_Policy_Matrix;
